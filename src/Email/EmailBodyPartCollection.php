    ...
    implements SerializationInterface, JsonSerializable
{
     ...
     * @param EmailBodyPart  $value The element to set.
     *
     * @return void
     */
    public function set($key, $value)
    {
        if (!$value instanceof EmailBodyPart) {
            throw new InvalidArgumentException(
                sprintf(
                    '%s only accepts instances of %s. Tried to to set key %s.',
                    __FUNCTION__, EmailBodyPart::class, $key
                )
            );
        }
     ...
     *
     * @param EmailBodyPart $element The element to add.
     *
        ...
        if (!$value instanceof EmailBodyPart) {
            throw new InvalidArgumentException(
                sprintf(
                    ...
                    __FUNCTION__, EmailBodyPart::class
                )
            );
        }
    ...
    public function jsonSerialize()
    {
        ...
        foreach ($this as $v) { /* @var $v EmailBodyPart */
            $ret[] = $v->jsonSerialize();
        }
